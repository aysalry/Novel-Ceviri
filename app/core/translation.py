...
class Translation:
    ...
    def set_pause_request(self, pause_request):
    ...
    def translate_paragraph(self, paragraph):
        # Runs inside Handler's thread-pool executor (see core/handler.py),
        # never on the asyncio loop or the Qt GUI thread, so blocking here
        # with a plain sleep only holds up this one worker slot -- the
        # other concurrency_limit workers each hit the same gate on their
        # next paragraph and stop there too, pausing the whole file without
        # needing to interrupt an in-flight HTTP request.
        while self.pause_request() and not self.cancel_request():
            time.sleep(0.2)
        if self.cancel_request():
            raise TranslationCanceled(_('Translation canceled.'))
